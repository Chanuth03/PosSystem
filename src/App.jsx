      ...
      <div className="flex h-screen bg-gray-50/50 flex-col md:flex-row">
        ...
        <div className="md:hidden flex justify-between items-center bg-white p-4 border-b border-gray-200 z-30 relative shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold bg-gradient-to-r from-blue-700 to-blue-500 bg-clip-text text-transparent">MyPOS</h1>
          </div>
        ...
        {/* Sidebar */}
        <aside className={`fixed md:static inset-y-0 left-0 w-72 md:w-64 bg-white border-r border-gray-200 flex flex-col transition-transform duration-300 shadow-2xl md:shadow-sm z-50 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
          ...
          <nav className="flex-1 px-4 space-y-2 mt-4 overflow-y-auto">
            ...
            <NavItem to="/pos" icon={<ShoppingCart size={20} />} label="Point of Sale" onClick={() => setIsSidebarOpen(false)} />
            <NavItem to="/inventory" icon={<Package size={20} />} label="Inventory" onClick={() => setIsSidebarOpen(false)} />
            <NavItem to="/vendors" icon={<Users size={20} />} label="Vendors" onClick={() => setIsSidebarOpen(false)} />
          </nav>
